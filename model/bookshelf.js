...
getBookshelfsCategory(name) {
  return new Promise((resolve, reject) => {
    this.connection.query(`    select * from Sach join theloai on sach.theloai_id = theloai.id
    join tinhtrangsach on sach.tinhtrang_id = tinhtrangsach.id
     where tentheloai like '%${name.search}%'
			or tinhtrang like '%${name.search}%'`, (err, data) => {
      if (err) {
        reject(err);
      } else {
        resolve(data);
...
setBooksCategory() {
  return new Promise((resolve, reject) => {
    let query = `select TenTheLoai,TenSach,MoTa,TinhTrang,HinhAnh from Sach join theloai on sach.theloai_id = theloai.id
    join tinhtrangsach on sach.tinhtrang_id = tinhtrangsach.id`
  this.connection.query(query,(err,data)=> {
    if(err) {
      reject(err)
